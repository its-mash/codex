...
impl ChatWidget {
    ...
    /// message or a native automation (loop/cron/monitor) firing — so the
    /// operator sees incoming team traffic instead of only the model's reaction.
    pub(super) fn render_inbound_inter_agent(&mut self, item: &codex_protocol::models::ResponseItem) {
...
#[cfg(test)]
mod inbound_inter_agent_tests {
    use super::inbound_inter_agent_notice;

    #[test]
    fn teammate_message_shows_sender_and_head() {
        let notice =
            inbound_inter_agent_notice("/root/external/team_lead", "PING_NATIVE").unwrap();
