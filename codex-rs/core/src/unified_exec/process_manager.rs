...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn write_stdin(
        &self,
        context: WriteStdinContext<'_>,
        request: WriteStdinRequest<'_>,
    ) -> Result<ExecCommandToolOutput, UnifiedExecError> {
        let process_id = request.process_id;
        let context = match context {
            WriteStdinContext::ToolCall(context) => Some(context),
            WriteStdinContext::BackgroundPoll if request.input.is_empty() => None,
            WriteStdinContext::BackgroundPoll => {
                return Err(UnifiedExecError::BackgroundPollInput);
            }
        };
        ...
        let approval = if let Some(context) = context {
            ...
            approval.map(|approval| (context, strict_auto_review, approval))
        } else {
            None
        };
        if let Some((context, strict_auto_review, (approval, approval_reason))) = approval {
